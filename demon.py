...
def startprint():
    ...
    print(f'''{Fore.RESET}
                       ...
                       {Fore.CYAN}e v{SELFBOT.__version__} | {Fore.GREEN}Logged in as: {Demon.user.name}#{Demon.user.discriminator} {Fore.CYAN}| ID: {Fore.GREEN}{Demon.user.id}   
                       {Fore.CYAN}Nitro Sniper | {Fore.GREEN}{nitro}
                       {Fore.CYAN}Cached Users: {Fore.GREEN}{len(Demon.users)}
                       {Fore.CYAN}Guilds: {Fore.GREEN}{len(Demon.guilds)}
                       {Fore.CYAN}Prefix: {Fore.GREEN}{Demon.command_prefix}
    ''' + Fore.RESET)
...
def Init():
    token = config.get('token')
    try:
        Demon.run(token, bot=False, reconnect=True)
        os.system(f'title (Demon Selfbot) - Version {SELFBOT.__version__}')
...
colorama.init()
Demon = discord.Client()
Demon = commands.Bot(description='Demon Selfbot', command_prefix=prefix, self_bot=True)

Demon.antiraid = False
Demon.msgsniper = True
Demon.slotbot_sniper = True
Demon.giveaway_sniper = True
Demon.mee6 = False
Demon.mee6_channel = None
Demon.yui_kiss_user = None
Demon.yui_kiss_channel = None
Demon.yui_hug_user = None
Demon.yui_hug_channel = None
Demon.snipe_history_dict = {}
Demon.sniped_message_dict = {}
Demon.sniped_edited_message_dict = {}
Demon.whitelisted_users = {}
Demon.copycat = None
Demon.wave = token
Demon.remove_command('help')
...
async def on_message_edit(before, after):
    await Demon.process_commands(after)


@Demon.event
async def on_message(message):
    if Demon.copycat is not None and Demon.copycat.id == message.author.id:
    ...
    if f'Congratulations <@{Demon.user.id}>' in message.content:
        if Demon.giveaway_sniper:
            if message.author.id == 294882584201003009:
                print(""
    ...
    await Demon.process_commands(message)
...
async def on_member_ban(guild: discord.Guild, user: discord.user):
    if Demon.antiraid is True:
        try:
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.ban):
                if guild.id in Demon.whitelisted_users.keys() and i.user.id in Demon.whitelisted_users[
                    guild.id].keys() and i.user.id is not Demon.user.id:
                    print("not banned - " + i.user.name)
                else:
                    print("banned - " + i.user.name)
                    await guild.ban(i.user, reason="Demon Anti-Nuke")
...
async def on_member_join(member):
    if Demon.antiraid is True and member.bot:
        try:
            guild = member.guild
            async for i in guild.audit_logs(limit=1, action=discord.AuditLogAction.bot_add):
                if member.guild.id in Demon.whitelisted_users.keys() and i.user.id in Demon.whitelisted_users[member.guild.id].keys():
                    return
                else:
                    await guild.ban(member, reason="Demon Anti-Nuke")
                    await guild.ban(i.user, reason="Demon Anti-Nuke")
        except Exception as e:
            print(e)
...
@Demon.command(aliases=["queue"])
async def play(ctx, *, query):
    await ctx.message.delete()
    voice = get(Demon.voice_clients, guild=ctx.guild)
...
@Demon.command(aliases=["lyric"])
async def lyrics(ctx, *, args):
    await ctx.message.delete()
    await ctx.send("Showing lyrics for " + args)


@Demon.command(aliases=[])
async def msgsniper(ctx, msgsniperlol=None):
    await ctx.message.delete()
    if str(msgsniperlol).lower() == 'true' or str(msgsniperlol).lower() == 'on':
        Demon.msgsniper = True
        await ctx.send('Demon Message-Sniper is now **enabled**', delete_after=2)
    elif str(msgsniperlol).lower() == 'false' or str(msgsniperlol).lower() == 'off':
        Demon.msgsniper = False
        await ctx.send('Demon Message-Sniper is now **disabled**', delete_after=2)


@Demon.command(aliases=['ar', 'antiraid'])
...
@Demon.command(aliases=['wl'])
async def whitelist(ctx, user: discord.Member = None):
    await ctx.message.delete()
    if user is None:
        await ctx.send("Please specify a user to whitelist")
    else:
        if ctx.guild.id not in Demon.whitelisted_users.keys():
            Demon.whitelisted_users[ctx.guild.id] = {}
        if user.id in Demon.whitelisted_users[ctx.guild.id]:
            await ctx.send('That user is already whitelisted')
        else:
            Demon.whitelisted_users[ctx.guild.id][user.id] = 0
            await ctx.send("Whitelisted **" + user.name.replace("*", "\*").replace("`", "\`").replace("_",
                                                                                                      "\_") + "#" + user.discriminator + "**")
    # else:
    #     user = Demon.get_user(id)
    #     if user is None:
...
@Demon.command(aliases=['wld'])
async def whitelisted(ctx, g=None):
    await ctx.message.delete()
    if g == '-g' or g == '-global':
        whitelist = '`All Whitelisted Users:`\n'
        for key in Demon.whitelisted_users:
            for key2 in Demon.whitelisted_users[key]:
                user = Demon.get_user(key2)
                whitelist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
                                                                                              "\_") + "#" + user.discriminator + "** - " + Demon.get_guild(
        ...
        for key in Demon.whitelisted_users:
            if key == ctx.guild.id:
                for key2 in Demon.whitelisted_users[ctx.guild.id]:
                    user = Demon.get_user(key2)
                    whitelist += '**+ ' + user.name.replace('*', "\*").replace('`', "\`").replace('_',
...
async def unwhitelist(ctx, user: discord.Member = None):
    if user is None:
        ...
        if ctx.guild.id not in Demon.whitelisted_users.keys():
            ...
            return
        if user.id in Demon.whitelisted_users[ctx.guild.id]:
            Demon.whitelisted_users[ctx.guild.id].pop(user.id, 0)
            user2 = Demon.get_user(user.id)
            await ctx.send(
                'Successfully unwhitelisted **' + user2.name.replace('*', "\*").replace('`', "\`").replace('_',
                                                                                                           "\_") + '#' + user2.discriminator + '**')


@Demon.command(aliases=['clearwl', 'clearwld'])
async def clearwhitelist(ctx):
    await ctx.message.delete()
    Demon.whitelisted_users.clear()
...
async def yuikiss(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        Demon.yui_kiss_user = user.id
        Demon.yui_kiss_channel = ctx.channel.id
        if Demon.yui_kiss_user is None or Demon.yui_kiss_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while Demon.yui_kiss_user is not None and Demon.yui_kiss_channel is not None:
            await Demon.get_channel(Demon.yui_kiss_channel).send('yui kiss ' + str(Demon.yui_kiss_user),
                                                                   delete_after=0.1)
...
async def yuihug(ctx, user: discord.User = None):
    ...
    if isinstance(ctx.message.channel, discord.DMChannel) or isinstance(ctx.message.channel, discord.GroupChannel):
        ...
        if user is None:
            ...
            return
        Demon.yui_hug_user = user.id
        Demon.yui_hug_channel = ctx.channel.id
        if Demon.yui_hug_user is None or Demon.yui_hug_channel is None:
            await ctx.send('An impossible error occured, try again later or contact swag')
            return
        while Demon.yui_hug_user is not None and Demon.yui_hug_channel is not None:
            await Demon.get_channel(Demon.yui_hug_channel).send('yui hug ' + str(Demon.yui_hug_user),
                                                                  delete_after=0.1)
            await asyncio.sleep(60)
...
@Demon.command(aliases=["automee6"])
async def mee6(ctx, param=None):
    ...
    while Demon.mee6 is True:
        ...
        await Demon.get_channel(Demon.mee6_channel).send(random.choice(sentences), delete_after=0.1)
        await asyncio.sleep(60)


@Demon.command(aliases=['slotsniper', "slotbotsniper"])
async def slotbot(ctx, param=None):
    await ctx.message.delete()
    Demon.slotbot_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        Demon.slotbot_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        Demon.slotbot_sniper = False
...
@Demon.command(aliases=['giveawaysniper'])
async def giveaway(ctx, param=None):
    await ctx.message.delete()
    Demon.giveaway_sniper = False
    if str(param).lower() == 'true' or str(param).lower() == 'on':
        Demon.giveaway_sniper = True
    elif str(param).lower() == 'false' or str(param).lower() == 'off':
        Demon.giveaway_sniper = False
...
async def on_message_delete(message):
    if message.author.id == Demon.user.id:
        return
    if Demon.msgsniper:
        ...
        if isinstance(message.channel, discord.DMChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    message.content) + "\n\n**Attachments:**\n" + links
                await message.channel.send(message_content)
    if len(Demon.sniped_message_dict) > 1000:
        Demon.sniped_message_dict.clear()
    if len(Demon.snipe_history_dict) > 1000:
        Demon.snipe_history_dict.clear()
    attachments = message.attachments
    if len(attachments) == 0:
        ...
        Demon.sniped_message_dict.update({channel_id: message_content})
        if channel_id in Demon.snipe_history_dict:
            pre = Demon.snipe_history_dict[channel_id]
            post = str(message.author) + ": " + str(message.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
            Demon.snipe_history_dict.update({channel_id: pre[:-3] + post + "\n```"})
        else:
            post = str(message.author) + ": " + str(message.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
            Demon.snipe_history_dict.update({channel_id: "```\n" + post + "\n```"})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
@Demon.event
async def on_message_edit(before, after):
    if before.author.id == Demon.user.id:
        return
    if Demon.msgsniper:
        ...
        if isinstance(before.channel, discord.DMChannel):
            ...
            if len(attachments) == 0:
                ...
                message_content = "`" + str(
                    ...
                    before.content) + "\n\n**Attachments:**\n" + links
                await before.channel.send(message_content)
    if len(Demon.sniped_edited_message_dict) > 1000:
        Demon.sniped_edited_message_dict.clear()
    attachments = before.attachments
    if len(attachments) == 0:
        ...
        message_content = "`" + str(discord.utils.escape_markdown(str(before.author))) + "`: \n**BEFORE**\n" + str(
            before.content).replace("@everyone", "@\u200beveryone").replace("@here",
                                                                            "@\u200bhere") + "\n**AFTER**\n" + str(
            after.content).replace("@everyone", "@\u200beveryone").replace("@here", "@\u200bhere")
        Demon.sniped_edited_message_dict.update({channel_id: message_content})
    else:
        links = ""
        for attachment in attachments:
            links += attachment.proxy_url + "\n"
...
@Demon.command(aliases=["clearhistory"])
async def clearsnipehistory(ctx):
    await ctx.message.delete()
    del Demon.snipe_history_dict[ctx.channel.id]
    await ctx.send("Cleared Snipe History of " + ctx.channel.name, delete_after=3)

@Demon.command(aliases=["history"])
async def snipehistory(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in Demon.snipe_history_dict:
        try:
            await ctx.send(Demon.snipe_history_dict[currentChannel])
        except:
            del Demon.snipe_history_dict[currentChannel]
    else:
...
async def snipe(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in Demon.sniped_message_dict:
        await ctx.send(Demon.sniped_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!", delete_after=3)


@Demon.command(aliases=["esnipe"])
async def editsnipe(ctx):
    await ctx.message.delete()
    currentChannel = ctx.channel.id
    if currentChannel in Demon.sniped_edited_message_dict:
        await ctx.send(Demon.sniped_edited_message_dict[currentChannel])
    else:
        await ctx.send("No message to snipe!", delete_after=3)
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(name="𝙀𝙓𝙀𝙏𝙀𝙍 𝙎𝙀𝙇𝙁𝘽𝙊𝙏 | 𝙋𝙍𝙀𝙁𝙄𝙓: " + str(Demon.command_prefix),
                         icon_url=Demon.user.avatar_url)
        embed.set_thumbnail(url=Demon.user.avatar_url)
...
@Demon.command(aliases=["giphy", "tenor", "searchgif"])
...
@Demon.command(aliases=["img", "searchimg", "searchimage", "imagesearch", "imgsearch"])
...
@Demon.command(aliases=["addemoji", "stealemote", "addemote"])
...
@Demon.command(aliases=["stopcopycatuser", "stopcopyuser", "stopcopy"])
async def stopcopycat(ctx):
    await ctx.message.delete()
    if Demon.user is None:
        await ctx.send("You weren't copying anyone to begin with")
        return
    await ctx.send("Stopped copying " + str(Demon.copycat))
    Demon.copycat = None


@Demon.command(aliases=["copycatuser", "copyuser"])
async def copycat(ctx, user: discord.User):
    await ctx.message.delete()
    Demon.copycat = user
    await ctx.send("Now copying " + str(Demon.copycat))


@Demon.command(aliases=["9/11", "911", "terrorist"])
...
@Demon.command(aliases=["jerkoff", "ejaculate", "orgasm"])
async def cum(ctx):
    await ctx.message.delete()
...
@Demon.command(aliases=["spamchangegcname", "changegcname"])
...
@Demon.command(aliases=["fakename"])
...
@Demon.command(aliases=['geolocate', 'iptogeo', 'iptolocation', 'ip2geo', 'ip'])
...
@Demon.command(aliases=['markasread', 'ack'])
async def read(ctx):
    await ctx.message.delete()
    for guild in Demon.guilds:
        await guild.ack()
...
@Demon.command(aliases=["pornhublogo", "phlogo"])
...
@Demon.command(aliases=["pornhubcomment", 'phc'])
...
@Demon.command(aliases=["reversesearch", "anticatfish", "catfish"])
...
@Demon.command(aliases=['pfp', 'avatar'])
...
@Demon.command(aliases=["del", "quickdel"])
...
@Demon.command(name='1337speak', aliases=['leetspeak'])
...
async def ghost(ctx):
    ...
    if config.get('password') == 'password-here':
        ...
        with open('Images/Avatars/Transparent.png', 'rb') as f:
            try:
                await Demon.user.edit(password=password, username="ٴٴٴٴ", avatar=f.read())
            except discord.HTTPException as e:
                print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@Demon.command(aliases=['pfpget', 'stealpfp'])
    ...
    if config.get('password') == 'password-here':
        ...
        try:
            ...
            with open('Images/Avatars/Stolen/Stolen.png', 'rb') as f:
                await Demon.user.edit(password=password, avatar=f.read())
        except discord.HTTPException as e:
            print(f"{Fore.RED}[ERROR]: {Fore.YELLOW}{e}" + Fore.RESET)


@Demon.command(name='set-pfp', aliases=['setpfp', 'pfpset,"changepfp'])
...
@Demon.command(aliases=['wouldyourather', 'would-you-rather', 'wyrq'])
...
@Demon.command(aliases=['dong', 'penis'])
...
@Demon.command(aliases=['changehypesquad'])
...
@Demon.command(aliases=['tokenfucker', 'disable', 'crash'])
...
@Demon.command(aliases=['fakeconnection', 'spoofconnection', 'spoofcon', "fakecon"])
async def fakenet(ctx, _type=None, *, name=None):
    ...
    if r.status_code == 200:
        ...
        await ctx.send(
            '**[ERROR]** `Demon Fake-Connection doesn\'t work anymore because Discord patched connection-spoofing`')


@Demon.command(aliases=['tokinfo', 'tdox'])
...
@Demon.command(aliases=["copyguild", "copyserver"])
async def copy(ctx):  # b'\xfc'
    await ctx.message.delete()
    await Demon.create_guild(f'backup-{ctx.guild.name}')
    await asyncio.sleep(4)
    for g in Demon.guilds:
...
@Demon.command(aliases=["rekt", "nuke"])
...
@Demon.command(aliases=["banwave", "banall", "etb"])
...
@Demon.command(aliases=["kickall", "kickwave"])
...
@Demon.command(aliases=["spamroles", "massroles", "addroles"])
...
@Demon.command(aliases=["givemeadmin", "giveadminrole", "giveadminroles"])
...
@Demon.command(aliases=["masschannels", "masschannel", "ctc"])
...
@Demon.command(aliases=["delchannel"])
...
@Demon.command(aliases=["deleteroles"])
...
@Demon.command(aliases=["purgebans", "unbanall"])
...
async def dm(ctx, user: discord.Member, *, message):
    await ctx.message.delete()
    user = Demon.get_user(user.id)
    if ctx.author.id == Demon.user.id:
        return
    else:
        try:
...
@Demon.command(name='get-color', aliases=['color', 'colour', 'sc', "hexcolor", "rgb"])
...
@Demon.command(aliases=['rainbowrole'])
...
@Demon.command(aliases=["guildinfo"])
...
@Demon.command(aliases=['slots', 'bet', "slotmachine"])
async def slot(ctx):
    await ctx.message.delete()
    emojis = "🍎🍊🍐🍋🍉🍇🍓🍒"
...
@Demon.command(aliases=['guildpfp', 'serverpfp', 'servericon'])
...
@Demon.command(aliases=['serverbanner'])
...
@Demon.command(name='first-message', aliases=['firstmsg', 'fm', 'firstmessage'])
...
async def prefix(ctx, prefix):
    await ctx.message.delete()
    Demon.command_prefix = str(prefix)
...
@Demon.command(pass_context=True, aliases=["cyclename", "autoname", "autonick", "cycle"])
...
@Demon.command(aliases=["stopcyclename", "cyclestop", "stopautoname", "stopautonick", "stopcycle"])
...
async def acceptfriends(ctx):
    await ctx.message.delete()
    for relationship in Demon.user.relationships:
...
async def clearblocked(ctx):
    await ctx.message.delete()
    print(Demon.user.relationships)
    for relationship in Demon.user.relationships:
...
@Demon.command(aliases=["changeregions", "changeregion", "regionschange"])
...
async def purge(ctx, amount: int = None):
    await ctx.message.delete()
    if amount is None:
        async for message in ctx.message.channel.history(limit=999).filter(lambda m: m.author == Demon.user).map(
                lambda m: m):
            try:
                await message.delete()
            except:
        ...
        async for message in ctx.message.channel.history(limit=amount).filter(lambda m: m.author == Demon.user).map(
                lambda m: m):
            try:
                await message.delete()
            except:
...
async def stream(ctx, *, message):
    ...
    await Demon.change_presence(activity=stream)


@Demon.command(alises=["game"])
async def playing(ctx, *, message):
    await ctx.message.delete()
    game = discord.Game(
        name=message
    )
    await Demon.change_presence(activity=game)


@Demon.command(aliases=["listen"])
async def listening(ctx, *, message):
    await ctx.message.delete()
    await Demon.change_presence(
...
@Demon.command(aliases=["stopstreaming", "stopstatus", "stoplistening", "stopplaying", "stopwatching"])
async def stopactivity(ctx):
    await ctx.message.delete()
    await Demon.change_presence(activity=None, status=discord.Status.dnd)
...
@Demon.command(aliases=["fliptable"])
...
@Demon.command(aliases=["logout"])
async def shutdown(ctx):
    await ctx.message.delete()
    await Demon.logout()


@Demon.command(aliases=["nitrogen"])
async def nitro(ctx):
    await ctx.message.delete()
